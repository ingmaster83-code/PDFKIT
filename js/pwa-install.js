...
(function() {
  ...
  window.showDownloadAd = function() {
    ...
    style.textContent = `
      ...
      #dl-ad-close {
        ...
        border:none; border-radius:8px; cursor:pointer; font-size:.83rem;
        display:none;
      }
  ...
  window.showPWABanner = function() {
    ...
    // iOS는 beforeinstallprompt가 없으므로 안내만
    ...
    if (!isIOS && !deferredPrompt) return; // 안드로이드인데 이미 설치됨

    const style = document.createElement('style');
    style.textContent = `
      #pwa-install-banner {
        position: fixed; bottom: 0; left: 0; right: 0; z-index: 9999;
        background: #1A1A2E; border-top: 3px solid #FF4444;
      ...
      .pwa-btn-install:hover { background: #e03333; }
      .pwa-btn-close {
